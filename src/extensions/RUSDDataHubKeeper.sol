...
import {IRUSDDataHub} from "src/interface/IRUSDDataHub.sol";
import {IRUSD} from "src/interface/IRUSD.sol";
...
abstract contract RUSDDataHubKeeper is Initializable, Base {
    ...
    modifier noPause() {
        if (PausableUpgradeable(address(getRUSDDataHub())).paused()) {
    ...
    modifier onlyAdapter() {
        if (msg.sender != getRUSDDataHub().getOmnichainAdapter()) {
            revert Unauthorized();
        }
        _;
    }
